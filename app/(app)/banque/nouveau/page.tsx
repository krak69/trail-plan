...
export default function NouvelExercicePage({
  ...
  return (
    <main className="mx-auto flex w-full max-w-lg flex-col gap-6 px-5 py-8 sm:px-8">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-xl">Nouvel exercice</CardTitle>
        </CardHeader>
        <CardContent>
          <form className="flex flex-col gap-4">
            <div className="flex flex-col gap-2">
              <Label htmlFor="name">Nom</Label>
              <Input id="name" name="name" placeholder="Fentes bulgares" required />
            </div>

            <div className="flex flex-col gap-2">
              <Label htmlFor="category">Catégorie</Label>
              <select id="category" name="category" defaultValue="" className={selectClasses} required>
            ...
            <div className="flex flex-col gap-2">
              ...
              <Input id="muscles" name="muscles" placeholder="Quadriceps, Fessiers" />
            </div>

            <div className="flex gap-4">
              <div className="flex flex-1 flex-col gap-2">
                <Label htmlFor="metric_value">Valeur</Label>
                <Input id="metric_value" name="metric_value" placeholder="12" />
              </div>
              <div className="flex flex-1 flex-col gap-2">
                <Label htmlFor="metric_unit">Unité</Label>
                <Input id="metric_unit" name="metric_unit" placeholder="rép / jambe" />
              </div>
            </div>

            <div className="flex flex-col gap-2">
              <Label htmlFor="tempo_tag">Tempo / tag (optionnel)</Label>
              <Input id="tempo_tag" name="tempo_tag" placeholder="Excentrique 3s" />
            </div>

            <div className="flex flex-col gap-2">
              <Label htmlFor="cue">Consigne</Label>
              <Textarea id="cue" name="cue" placeholder="Descente lente et contrôlée, genou dans l'axe du pied." />
            </div>
            ...
            <Button type="submit" formAction={createExercise} className="mt-2 w-full">
